...
{
    public class u8DispatchMain : u8<VouchMain>
    {
        ...
        private string whereStr(VouchMain searchKey)
        {
            ...
            if (string.IsNullOrEmpty(searchKey.vouchCode))
                wStr.Append(" and cDlCode like '%"+searchKey.vouchCode+"%'");
            if (searchKey.Mid > 0)
            ...
            if (string.IsNullOrEmpty(searchKey.cShipAddress))
                wStr.Append(" and cShipAddress like '%"+searchKey.cShipAddress+"%'");
        ...
        private string headSqlCmd()
        {
            ...
            sqlcmd.Append("select dlid, cdlcode, ccusCode, dDate ");
            sqlcmd.Append(" , dCreateSystime, dmodifySystime, cModifier ");
            sqlcmd.Append(" , cShipAddress, cMaker, cVerifier, cMemo, ccusperson ");
            sqlcmd.Append(" from DispatchList where 1 = 1 ");
            return sqlcmd.ToString();
        }
        private void vmMapper(VouchMain vm,IDataReader row)
        {
            vm.vouchCode = row.GetString("cDlCode");
            vm.Mid = Convert.ToInt32(row.GetValue("dlid"));
            vm.vouchDate = row.GetDateTime("dDate");
            vm.corporatio = new u8Customer().getSingle(row.GetString("ccusCode"));
            vm.cShipAddress = row.GetString("cShipAddress");
        ...
        public u8DispatchMain() {
            Fields = new List<string>() { "dlid", "cdlcode", "ccusCode", "dDate", "dCreateSystime", "dmodifySystime", "cModifier ", "cShipAddress", "cMaker", "cVerifier", "cMemo", "ccusperson" };
        }
        ...
        public override VouchMain getSingle(string code)
        {
            sqlcmd = new StringBuilder();
            sqlcmd.Append(headSqlCmd());
            sqlcmd.Append(" and cdlcode = '"+code+"'");
        ...
        public override void setField(string field, string val, string whereStr)
        {
            //only Modify define
            sqlcmd = new StringBuilder();
            sqlcmd.Append("Update DispatchList");
        ...
        public override object getField(string field, VouchMain searchKey)
        {
            object r = null;
            sqlcmd = new StringBuilder();
            sqlcmd.Append(" select " + field + " from dispatchlist where 1 = 1 ");
