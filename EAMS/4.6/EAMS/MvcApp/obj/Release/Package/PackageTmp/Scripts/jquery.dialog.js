...
SubmitCompleted2:function(alertMsg, parentUrl, params){if($.trim(alertMsg).length > 0 ){alert(alertMsg);}JqueryDialog.Close();$('body').load(parentUrl,params);},
...
SubmitCompleted1:function(alertMsg, parentUrl){if($.trim(alertMsg).length > 0 ){alert(alertMsg);}JqueryDialog.Close();window.location.href = parentUrl;}
};
