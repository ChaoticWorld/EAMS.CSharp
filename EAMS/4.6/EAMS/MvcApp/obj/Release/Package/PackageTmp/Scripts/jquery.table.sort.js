    ...
    sorttable: function (setting) {
        ...
        function SetColumnOrder(headCell) {
            ...
            rs.sort(function (r1, r2) {
                var acell = $(r1).children("td").eq(index);
                var bcell = $(r2).children("td").eq(index);
                var a = $.trim($(acell).text());
                var b = $.trim($(bcell).text());

                //新增,当单元格内不为text时,取单元格中children[0].val()
                if ((!a) && $.trim($($(acell).children()[0]).val())) {
                    a = $.trim($($(acell).children()[0]).val());
                }
                if ((!b) && $.trim($($(bcell).children()[0]).val())) {
                    b = $.trim($($(bcell).children()[0]).val());
                }
