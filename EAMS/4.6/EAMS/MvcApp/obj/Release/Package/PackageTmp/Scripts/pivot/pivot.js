(function () {
    ...
    callWithJQuery(function ($) {
        ...
        pivotTableRenderer = function (pivotData, opts) {
			...
			opts.aggregateDeep = rowAttrs.length-1;
            result = document.createElement("table");
            result.className = "pvtTable";
            subTotalRow = function (i, j) {
                var newSubTotalRow,
                rKey,
                cKey,
                subSumVal,
                subSumTh,
                subSumTd;
                newSubTotalRow = document.createElement("tr");
                subSumTh = document.createElement("th");
                subSumTh.className = "pvtTotalLabel pvtRowLabel";
                subSumTh.innerHTML = opts.localeStrings.subTotals;
                subSumTh.setAttribute("colspan", rowAttrs.length - j);
                newSubTotalRow.appendChild(subSumTh);
                var filterRows = [];
                filterRows.push(rowKeys[i]);
                while (i >= 1) {
                    v1 = "";
                    v2 = "";
                    for (ki = j; ki >= 0; ki--) {
                ...
                for (ci in colKeys) {
                    if (!hasProp.call(colKeys, ci))
                        continue;
                    cKey = colKeys[ci];
                    subSumVal = 0;
                    for (ri in filterRows) {
                        if (!hasProp.call(filterRows, ri))
                            continue;
                        rKey = filterRows[ri];
                        aggregator = pivotData.getAggregator(rKey, cKey);
                        subSumVal += aggregator.value();
                    }
                    subSumTd = document.createElement("td");
                    subSumTd.className = "pvtTotalLabel pvtVal rowSubTotal col" + ci;
                    subSumTd.textContent = subSumVal;
                    subSumTd.setAttribute("data-value", subSumVal);
                    newSubTotalRow.appendChild(subSumTd);

                }
                //小计行最后列小计
                var val = 0;
                $(newSubTotalRow).children("td").each(function () {
                    var v = parseInt($(this).text());
                    val += (isNaN(v) ? 0 : v);
                });
                subSumTd = document.createElement("td");
                subSumTd.className = "pvtTotalLabel pvtTotal rowTotal";
                subSumTd.textContent = val;
                subSumTd.setAttribute("data-value", val);
                subSumTd.setAttribute("data-for", "subTotalRow");
                newSubTotalRow.appendChild(subSumTd);
            ...
            spanDeep = function (arr, i, j, x) {
                var compKey = arr[i];
                var deep = 0;
                var v1 = "",v2 = "";
				if (j < opts.aggregateDeep && j < compKey.length)  deep=1;
                for (di = j; di < opts.aggregateDeep && di < compKey.length; di++) {                    
                    for (ri = i + 1; ri < i + x && ri < arr.length; ri++) {
						v1 = "";v2 = "";
                        for (ki=di;ki>=0;ki--){
							v1 += arr[ri - 1][ki];
							v2 += arr[ri][ki];
            ...
            for (i in rowKeys) {
                ...
                for (j in rowKey) {
                    ...
                    v2 = "";
                    if (x !== -1) {
                        if (i > 0 && j < opts.aggregateDeep && subSumTotalOnce++ <= 0) { 
                            for (k = opts.aggregateDeep - 1; k >= 0; k--) {
                                v1 = "";
                                v2 = "";
                                for (ki = k; ki >= 0; ki--) {
                                    v1 += rowKeys[i][ki];
                                ...
                                if (v1 != v2) {
                                    result.appendChild(subTotalRow(i - 1, k));
                                }
                            }
                        }
                        th = document.createElement("th");
                        ...
                        th.textContent = txt;						
                        deep1 = spanDeep(rowKeys, parseInt(i), parseInt(j), x);
                        if (deep1 > 1)
                            th.setAttribute("rowspan", deep1);
			...
			//补充最后小计
			for (l=opts.aggregateDeep-1;l>=0;l--) {
			result.appendChild(subTotalRow(rowKeys.length-1, l));
			}
			//总计
            tr = document.createElement("tr");
            th = document.createElement("th");
